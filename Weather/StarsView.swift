...
struct StarsView: View {
    ...
    var meteorShowerIntensity: Double
    
    init(meteorShowerIntensity: Double) {
        self.meteorShowerIntensity = meteorShowerIntensity
        meteorShower.intensity = meteorShowerIntensity
    }
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            Canvas { context, size in
                ...
                let rightColors: [Color] = [.clear, Color(red: 0.8, green: 1, blue: 1), .white]
                ...
                for meteor in meteorShower.meteors {
                    ...
                    contextCopy.fill(glow, with: .color(.white))
                }
                ...
                for (index, star) in starField.stars.enumerated() {
                    ...
                    if star.flickerInterval == 0 {
                        var flashLevel = abs(sin(Double(index) + timeInterval * 4))
                        flashLevel /= 2
        ...
        .onChange(of: meteorShowerIntensity) { oldValue, newValue in
            meteorShower.intensity = newValue
        }
    }
}

#Preview {
    StarsView(meteorShowerIntensity: 5)
}
