...
class Lightning {
    ...
    func update(date: Date, in size: CGSize) {
        ...
        case .waiting:
            break
        case .preparing:
            let startPosition = CGPoint(x: .random(in: 50...size.width-50), y: 0)
            let newBolt = LightningBolt(start: startPosition, width: 5, angle: Angle.degrees(-90).radians)
        ...
        case .striking:
            ...
            for bolt in bolts {
                ...
                for _ in 0..<5 {
                    let endX = lastPoint.x + (speed * cos(bolt.angle) + .random(in: -10...10))
                    let endY = lastPoint.y - (speed * sin(bolt.angle) + .random(in: -10...10))
                ...
                if lastPoint.y < size.height {
                    ...
                    if bolts.count < maximumBolts && Int.random(in: 0..<100) < forkProbability {
                        let newAngle = Angle.degrees(.random(in: -135 ... -45)).radians
